...
struct Skill {
    ...
    // Karuah Chess patch. As using negative skill levels, ensure min value is at least 1.
    bool time_to_pick(Depth depth) const { return depth == std::max(1 + int(level), 1); }
...
class SearchManager: public ISearchManager {
   public:
    ...
    // Karuah Chess - including vector for moves
    using UpdateBestmove = std::function<void(std::string_view, std::string_view, std::vector<Stockfish::Search::RootMove>)>;
