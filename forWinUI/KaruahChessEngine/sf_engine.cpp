...
Engine::Engine() :
    ...
    options.add("Skill Level", Option(20, -10, 20));
...
void Engine::set_on_bestmove(std::function<void(std::string_view, std::string_view, std::vector<Stockfish::Search::RootMove>)>&& f) {
...
void Engine::verify_networks() const {
    networks->big.verify();
    networks->small.verify();
}

void Engine::load_networks() {
    networks.modify_and_replicate([this](NN::Networks& networks_) {
        networks_.big.load();
        networks_.small.load();
    });
    threads.clear();
    threads.ensure_network_replicated();
}
